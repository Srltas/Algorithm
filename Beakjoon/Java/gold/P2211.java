/**
 *
 * URL : https://www.acmicpc.net/problem/2211
 *
 * 네트워크 복구
 *
 * 문제
  ...
  private void dijkstra() {
    ...
    while (!queue.isEmpty()) {
      ...
      for (Node next : list[curr.to]) {
        if (distance[next.to] > distance[curr.to] + next.speed) {
          distance[next.to] = distance[curr.to] + next.speed;
          queue.offer(new Node(next.to, distance[next.to]));
          prePath[next.to] = curr.to;
        }
      }
    }
  }
