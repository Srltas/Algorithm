...
public class P1414 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        PriorityQueue<lEdge> q = new PriorityQueue<>();
        int totalLenSun = 0;

        for (int i = 0; i < N; i++) {
            StringTokenizer st = new StringTokenizer(br.readLine());
            char[] lenSuns = st.nextToken().toCharArray();


            for (int j = 0; j < N; j++) {
                int lenSun = 0;

                if (lenSuns[j] >= 'a' && lenSuns[j] <= 'z') {
                    lenSun = lenSuns[j] - 'a' + 1;
                } else if (lenSuns[j] >= 'A' && lenSuns[j] <= 'Z') {
                    lenSun = lenSuns[j] - 'A' + 27;
                }
                totalLenSun += lenSun;
                // 자기 자신 컴퓨터가 아니면서 길이기 0이 아니면 우선순위 큐에 넣음
                if (i != j && lenSun != 0) {
                    q.offer(new lEdge(i, j, lenSun));
                }
            }
        }
        ...
        if (useEdge == N - 1) {
            System.out.println(totalLenSun - result);
        } else {
