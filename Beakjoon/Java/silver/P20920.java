...
public class P20920 {
  ...
  public void solution() throws IOException {
    ...
    Map<String, Word> wordNote = new HashMap<>();
    String str;
    for (int i = 0; i < N; i++) {
      str = br.readLine();
      if (str.length() < M) {
      ...
      Word newWord = wordNote.getOrDefault(str, new Word(str, 0));
      newWord.count++;
      wordNote.put(str, newWord);
    }

    List<Word> words = new ArrayList<>(wordNote.values());
    words.sort(null);

    for (Word w : words) {
      bw.write(w.word + System.lineSeparator());
    }
    bw.flush();
    bw.close();
    br.close();
