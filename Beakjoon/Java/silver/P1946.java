...
public class P1946 {
  ...
  public void solution() throws IOException {
    ...
    while (T-- > 0) {
      ...
      Volunteer[] array = new Volunteer[N];
      for (int i = 0; i < N; i++) {
        st = new StringTokenizer(br.readLine());
        int paper = Integer.parseInt(st.nextToken());
        int interview = Integer.parseInt(st.nextToken());
        array[i] = new Volunteer(paper, interview);
      }

      Arrays.sort(array);

      int count = 1;
      int minInterview = array[0].interview;
      for (int i = 1; i < N; i++) {
        if (array[i].interview < minInterview) {
          count++;
          minInterview = array[i].interview;
        }
      }
