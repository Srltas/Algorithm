...
public class P18110 {
  ...
  public void solution() throws IOException {
    ...
    Arrays.sort(array);
    int excludedPercentage = (int) Math.round(array.length * 0.15);
    int[] resultArray = Arrays.copyOfRange(array, excludedPercentage, array.length - excludedPercentage);
    int difficulty = (int) Math.round(Arrays.stream(resultArray).average().orElse(0));

    bw.write(difficulty + System.lineSeparator());
    bw.flush();
