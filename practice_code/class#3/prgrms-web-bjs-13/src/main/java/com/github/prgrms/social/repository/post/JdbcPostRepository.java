...
public class JdbcPostRepository implements PostRepository {
  ...
  @Override
  public Optional<Post> findById(Id<Post, Long> postId, Id<User, Long> userId, Id<User, Long> writerId) {
    // TODO likesOfMe 값을 효율적으로 구하기 위해 쿼리 튜닝
    List<Post> results = jdbcTemplate.query(
      "SELECT " +
                "p.*,u.email,u.name, ifnull(l.seq, false) as likesOfMe " +
              "FROM " +
                "posts p JOIN users u ON p.user_seq = u.seq LEFT OUTER JOIN likes l ON p.seq=l.post_seq AND l.user_seq=u.seq " +
              "WHERE " +
  ...
  public List<Post> findAll(Id<User, Long> writerId, Id<User, Long> readerId, long offset, int limit) {
    ...
    return jdbcTemplate.query(
      "SELECT " +
                "p.*, u.email, u.name, ifnull(l.seq, false) AS likesOfMe " +
              "FROM " +
                "posts p JOIN users u ON p.user_seq=u.seq LEFT OUTER JOIN likes l ON l.post_seq=p.seq AND l.user_seq=? " +
              "WHERE " +
                "p.user_seq=? " +
              "ORDER BY p.seq DESC " +
              "LIMIT ?,?",
      mapper,
      readerId.value(), writerId.value(), offset, limit
    );
  }
