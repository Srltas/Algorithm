...
public class UserRestController {
  ...
  public ApiResult<Boolean> checkEmail(@RequestBody Map<String, String> email) {
    // TODO 이메일 중복 확인 로직 구현
    // BODY 예시: {
    //	"email": "iyboklee@gmail.com"
    //}

    return OK(userService.findByEmail(new Email(email.get("email"))).isPresent());
  }

  @PostMapping(path = "user/join")
  public ApiResult<JoinResult> join(@RequestBody JoinRequest joinRequest) {
    User user = userService.join(new Email(joinRequest.getPrincipal()), joinRequest.getName(), joinRequest.getCredentials());
